...
public class Programme_5_PalindromeNumber {
    public static boolean isPalindrome(int number){
        // convers negative number to positive number
        if(number <0){
            number = -number;
             }
        int lastDigit, resverse =0, temp;
        temp = number;
        while(number>0){
            //Stroes the last digit
            lastDigit = number %10;
    ...
    public static void main(String [] args){
        //Scanner declaration fr reading input form console
        Scanner scanner = new Scanner(System.in);
        System.out.println("Enter any number: ");
        int a = scanner.nextInt();
        boolean check = isPalindrome(a);
        //checks number is Plindrome or not
        if(check)
